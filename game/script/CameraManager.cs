...
#region Enums
		...
		public bool CheckDirection(Direction Dir, float Ratio)
		{
			...
			ja2.TerrainTileHandle last_tile = new ja2.TerrainTileHandle(TerrainPartition.PARTITION_WIDTH - 1, TerrainPartition.PARTITION_HEIGHT - 1, m_TerrainManager.width - 1, m_TerrainManager.height - 1);
			...
			{
				case Direction.LEFT:
					ret = point_on_plane.z - amount_ratio > TerrainPartition.TILE_WIDTH;
					break;
				case Direction.TOP:
					ret = point_on_plane.x - amount_ratio > TerrainPartition.TILE_HEIGHT;
					break;
				case Direction.BOTTOM:
		...
		private void RecalculateCamera()
		{
			...
			Vector3 point_on_plane = ray.GetPoint(point);
			// Normalize position
			transform.Translate(0, 0, -point_on_plane.z + TerrainPartition.TILE_WIDTH, Space.World);
			transform.Translate(point_on_plane.x + TerrainPartition.TILE_HEIGHT, 0, 0, Space.World);
		}
